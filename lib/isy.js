...
class ISY {
    ...
    loadScenes(result) {
        for (let scene of result.nodes.group) {
            ...
            var newScene = new isyscene_1.ISYScene(this, sceneName, sceneAddress, childDevices);
    ...
    loadElkNodes(result) {
        ...
        p.parseString(result, (err, res) => {
            ...
            for (let nodes of res.areas.area.zone) {
                ...
                var newDevice = new elkdevice_1.ElkAlarmSensor(this, name, 1, id, (alarmDef == 17) ? "coSensor" : "alarmDoorWindowSensor");
    ...
    loadVariables(type, done) {
        ...
        var getInitialValuesCB = (result, response) => {
        ...
        var getVariableInitialValues = () => {
            ...
            // Load initial values
            restler.get(that.protocol + '://' + that.address + '/rest/vars/get/' + type, options).on('complete', getInitialValuesCB);
        };
        // Callback function to get the variable values after getting definitions
        var loadVariablesCB = (result, response) => {
            if (that.checkForFailure(response)) {
        ...
        // Load definitions
        restler.get(that.protocol + '://' + that.address + '/rest/vars/definitions/' + type, options).on('complete', loadVariablesCB);
    }
    getVariableList() {
        ...
        p.parseString(result, (err, res) => {
            ...
            if (res.CList.e) {
                for (let v of res.CList.e) {
                    var newVariable = new isyvariable_1.ISYVariable(this, v.id, v.name, type);
    ...
    initialize(initializeCompleted) {
        ...
        restler.get(this.protocol + '://' + this.address + '/rest/nodes', options).on('complete', (result, response) => {
            if (that.checkForFailure(response)) {
                ...
                that.loadVariables(ISYDefs.variableType.integer, () => {
                    that.loadVariables(ISYDefs.variableType.state, () => {
                        if (that.elkEnabled) {
                            restler.get(that.protocol + '://' + that.address + '/rest/elk/get/topology', options).on('complete', (result, response) => {
                                if (that.checkForFailure(response)) {
                                    ...
                                    restler.get(that.protocol + '://' + that.address + '/rest/elk/get/status', options).on('complete', (result, response) => {
                                        if (that.checkForFailure(response)) {
    ...
    initializeWebSocket() {
        ...
        this.webSocket = new WebSocket(this.wsprotocol + '://' + this.address + '/rest/subscribe', ['ISYSUB'], {
            headers: {
